...
def test_run_analysis_builds_one_case_per_study_and_one_summary_with_shot_iq(monkeypatch):
    ...
    def fake_run_analysis_stage(**kwargs):
        ...
        if trajectory is run_ground.results["param_0"].trajectories["shot_0"]:
...
def test_run_one_solver_study_uses_tag_as_run_key_and_sets_study_identity(monkeypatch):
    ...
    assert list(model.runs.keys()) == ["solver_0"]
    assert list(model.runs["solver_0"].keys()) == ["Relaxation"]
    assert model.runs["solver_0"]["Relaxation"].identity.run_id == "Relaxation"
    assert model.runs["solver_0"]["Relaxation"].identity.study_name == "Relaxation"
    assert model.runs["solver_0"]["Relaxation"].identity.study_index == 0
...
def test_run_one_solver_study_disambiguates_profiles_with_same_study_name(monkeypatch):
    ...
    assert run_ids == ["Relaxation__single_qubit_decoherence", "Dephasing__single_qubit_decoherence"]
    assert created_run_ids == run_ids
    assert executed_circuits == {
        "Relaxation__single_qubit_decoherence": "x",
        "Dephasing__single_qubit_decoherence": "h",
    }


def test_id_generator_skips_existing_run_and_analysis_ids():
    model = SimpleNamespace(
        runs={"solver_0": {"run_0": object(), "run_1": object()}},
