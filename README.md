...
MuSIQ is a workflow-first quantum simulation project with reproducible artifacts, QEC pipeline integration, and documentation automation.
...
- Schedule-first circuit IR (`CircuitIR.schedule`)
- Typed pulse recipes under `pulse.defaults`, `pulse.gates`, and `pulse.channel_overrides`
...
```bash
musiq run-task --task-config tasks/demo.json
```
...
- Typed pulse schema guide: `docs/src/wiki/pulse_schema.md`
- Schedule circuit IR guide: `docs/src/wiki/circuit_ir.md`

## Compiler Contracts

The compiler-facing resource formats are now:

- circuits:
  - OpenQASM input, or
  - direct schedule YAML with `format: circuit_layer_yaml`
- pulses:
  - typed gate-first config under `pulse.defaults`, `pulse.gates`, and `pulse.channel_overrides`

Legacy pulse schemas based on `channels` / `carriers` / `waveforms` /
`operations`, as well as legacy `amp_scale` recipe semantics, are intentionally
rejected.
