...
def _plot_pulses_timing(
    ...
    *,
    ...
    for yv, txt in zip(y_positions, y_labels):
        ...
        label_text.set_gid("musiq_channel_label")
    fig.tight_layout()
    fig._musiq_pulse_metadata = pulse_metadata  # type: ignore[attr-defined]
    return fig
...
def _export_timing_figure_to_dxf(fig, out_path: str | Path) -> Path:
    ...
    for t in ax.texts:
        if t.get_gid() != "musiq_channel_label":
            continue
        txt = t.get_text()
        if not txt:
            continue
...
def plot_pulses(
    ...
    if timing_layout:
        ...
        if pulse_metadata_path is not None:
            payload = {
                "schema": "musiq.pulse-metadata.v1",
                "count": len(getattr(fig, "_musiq_pulse_metadata", [])),
                "pulses": list(getattr(fig, "_musiq_pulse_metadata", [])),
            }
