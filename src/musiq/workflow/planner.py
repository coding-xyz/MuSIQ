...
from musiq.workflow.contracts import Task
...
def _task_field_value(task: Task, dotted: str):
    current = task
...
def _validate_required_fields(task: Task, required_fields: list[str]) -> None:
...
def _normalize_targets(task: Task) -> tuple[str, list[str]]:
...
def build_execution_plan(task: Task) -> ExecutionPlan:
