...
from musiq.analysis.error_budget_pauli import write_component_ablation_csv
from musiq.analysis.sensitivity import write_sensitivity_heatmap
from musiq.backend.config import dump_backend_config
from musiq.common.schemas import RunManifest, write_json
from musiq.pulse.drawer_adapter import EngineeringDrawer
from musiq.qec.eval import write_decoder_eval_csv, write_decoder_pareto_png, write_failed_tasks_jsonl
from musiq.qec.prior import write_prior_samples_npz
from musiq.workflow.engines import collect_runtime_dependencies
from musiq.workflow.output import export_circuit_diagram, export_result_figures, sha256_text, write_trajectory_h5
...
def export_visualizations(
    *,
    ...
    if export_dxf and (selected is None or "timing_diagram" in selected):
        try:
            EngineeringDrawer.export_dxf(pulse_ir, out / "timing_diagram.dxf", style={"title": "musiq timing"})
