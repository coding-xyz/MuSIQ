...
def rates_from_T1_T2(T1: float, T2: float, Tup: float | None = None) -> Tuple[float, float, float]:
    gamma_down = 0.0 if T1 <= 0 else 2*np.pi / T1
    gamma_up = 0.0 if not Tup or Tup <= 0 else 2*np.pi / Tup
    gamma_phi = max(0.0, (0.0 if T2 <= 0 else 2*np.pi / T2) - 0.5 * (gamma_down + gamma_up))
